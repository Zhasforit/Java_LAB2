...
public class IZT2_LAB2 {

    public static void main(String[] args) {
        try {
            // Адрес нашей базы данных "tsn_demo" на локальном компьютере (localhost)
            String url = "jdbc:mysql://localhost:3306/izt2_lab2?useSSL=false&serverTimezone=UTC";
            ...
            ResultSet table = statement.executeQuery("SELECT * FROM MONEY ");
            ...
            String scannedType = sc.nextLine();
            System.out.print("cost - ");
            String scannedCost = sc.nextLine();
            
            System.out.println("После добавления:");
            statement.execute("INSERT MONEY(type, cost) VALUES ('" + scannedType + "', '" + scannedCost + "')");
            table = statement.executeQuery("SELECT * FROM MONEY");
            ...
            statement.execute("DELETE FROM Money WHERE Id = " + scannedId);
            ...
            System.out.print("Введите условие(фильтр) - ");
            String scannedFilter = sca.nextLine();
            
            table = statement.executeQuery("SELECT * FROM MONEY WHERE " + scannedFilter);
            while (table.next()) {
                for (int j = 1; j <= table.getMetaData().getColumnCount(); j++) {
                    System.out.print(table.getString(j) + "\t\t");
                }
